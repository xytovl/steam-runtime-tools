...
G_LOCK_DEFINE_STATIC (path_cached);
#endif
  ...
  G_LOCK (path_cached);
    {
      ...
      if (path == NULL)
        {
          g_autofree char *new_path = g_file_get_path (file);

          path = new_path;

          if (new_path != NULL)
            g_object_set_qdata_full ((GObject *) file, _file_path_quark,
                                     g_steal_pointer (&new_path), g_free);
        }
    }
  G_UNLOCK (path_cached);
#endif
